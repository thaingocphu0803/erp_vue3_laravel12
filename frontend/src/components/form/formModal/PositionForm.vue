...
} = useFilterFetch(positionFetch, 'positionFetch')
...
<template>
	<Form :title @submit-form="handleSubmit">
		...
		<v-row dense>
			...
			<v-col cols="12" md="6">
				<Input :label="$t('position.input.positionCode')" name="code" v-model="positionFormData.code"
					:maxlength="CONFIG.maxLengthCode" counter>
					<template #append-inner>
						<annotation-tooltip text="position.tooltip.codeAutoGenerate"></annotation-tooltip>
					</template>
				</Input>
			</v-col>
		</v-row>
