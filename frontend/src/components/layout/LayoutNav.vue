			...
			<template v-for="item in navMenu" :key="item.value">
				<v-list-group v-if="item.children" :value="item.value">
					<template v-slot:activator="{ props }">
						<v-list-item v-bind="props" :title="$t(item.title)" :value="item.value"
							:prepend-icon="item.icon"></v-list-item>
					</template>

					<v-list-item v-for="subItem in item.children" :prepend-icon="subItem.icon" :key="subItem.value"
						:title="$t(subItem.title)" :to="{ name: subItem.routeName }"
						:value="subItem.value"></v-list-item>
				</v-list-group>

				<v-list-item v-else :prepend-icon="item.icon" :value="item.value" :to="{ name: item?.routeName }"
					:title="$t(item.title)" class="text-capitalize" data-testId="nav-menu-item">
				</v-list-item>
			</template>
		</v-list>
	</v-navigation-drawer>
